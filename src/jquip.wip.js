    var $ = jQuery = (function () {
        ...
        $.fn = $.prototype = {
            ...
            init: function (selector, ctx) {
                ...
                if (selector.charAt(0) === "<") return this.make(_fromHtml(selector));
        ...
        function $find(rootEl, tagSel) {
            ...
            if (tagWithId.length == 2) {
                ...
                if (!el) return [];
                if (!_eq(el.tagName, tagWithId[0])) return [];
        ...
        function _walk(ret, el, fn) {
            if (el.nodeType == 1)
                if (fn(el)) ret.push(el);
            var els = el.childNodes;
            for (var i = 0, l = els.length; i < l; i++) {
                var subEl = els[i];
                if (subEl.nodeType != 1) continue;
        ...
        var $cls = doc.getElementsByClassName //&& false
            ...
            : win.Sizzle || function (selector, ctx) {
                var els, resSet = [[(ctx || doc)]], args = selector.split(' '), ret = [];
                for (var i = 0, l = args.length; i < l; i++) {
                    var parentSet = resSet[i];
                    if (parentSet.length == 0) return ret; //short-circuit
                    var sel = args[i], res = [], firstChar = sel.charAt(0);
                    if (firstChar === '#') {
                        resSet.push([(ctx || doc).getElementById(sel.substring(1))]);
                        continue;
                    }
                    else if (firstChar == '.') {
                        resSet.push($cls(sel.substring(1), (ctx || doc)));
                        continue;
                    }
        ...
        nativeBind = FuncProto.bind, ctor = function () { }, //_.js
        ...
        function _bind(fn, ctx) {
            ...
            if (!_.isFunction(fn)) throw new TypeError;
            args = slice.call(arguments, 2);
            return bound = function () {
                if (!(this instanceof bound)) return fn.apply(ctx, args.concat(slice.call(arguments)));
        ...
        $.extend = $.fn.extend = function () {
            var opt, name, src, copy, copyIsArray, clone, args = arguments,
			target = args[0] || {}, i = 1, length = args.length, deep = false;
            if (typeof target === "boolean")
                deep = target, target = args[1] || {}, i = 2;
